    ...
    TimelineView(.animation) { timeline in
      ...
      Canvas { context, size in
        context.fill(
          ...
          with: .linearGradient(Gradient(colors: [.pink, .blue]), startPoint: CGPoint(x: 0, y: 0), endPoint: CGPoint(x: 400, y: 400))
        )
      }
...
extension BlobView {
  ...
  private func path(in rect: CGRect, x: Double = 1, x2: Double = 1) -> Path {
